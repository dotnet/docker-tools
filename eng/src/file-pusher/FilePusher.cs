        ...
        public static async Task PushFilesAsync(Options options, Config config)
        {
            ...
            foreach (GitRepo repo in GetFilteredRepos(config, options))
            {
                ...
                PullRequestSpec spec = new()
                {
                    ...
                    // These PRs are bot-owned: each run replaces the PR with the latest
                    // content, even if someone has pushed to the branch in the meantime.
                    OnForeignCommits = ForeignCommitPolicy.Overwrite,
                    Apply = repoRoot => CopyFilesAsync(config.SourcePath, repoRoot),
                };
