...
public class GenerateSigningPayloadsCommand : Command<GenerateSigningPayloadsOptions, GenerateSigningPayloadsOptionsBuilder>
{
    ...
    private async Task ExecuteAsyncInternal()
    {
        ...
        _loggerService.LogInformation("GENERATING SIGNING PAYLOADS");

        _loggerService.LogInformation("Reading digests from image info file");
        ...
        _loggerService.LogInformation("Writing payloads to disk");
        ...
        string outputDirectory)
    {
        ...
        _loggerService.LogInformation(taskCompletedMessage);
    }
    ...
    private async Task<string> WritePayloadToDiskAsync(Payload payload, string outputDirectory)
    {
        ...
        if (!Options.IsDryRun)
        {
            ...
            _loggerService.LogInformation($"Wrote signing payload to disk: {payloadPath}");
        }
        else
        {
            _loggerService.LogInformation($"Dry run: Would have written signing payload to disk: {payloadPath}");
        }
