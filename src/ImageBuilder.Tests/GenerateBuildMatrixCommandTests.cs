...
namespace Microsoft.DotNet.ImageBuilder.Tests
{
    public class GenerateBuildMatrixCommandTests
    {
            ...
            string inputPathFilters = "*runtime* *sdk*")
        {
            ...
            GenerateBuildMatrixCommand command = new(CreateManifestInfoProviderMock().Object, imageCacheServiceMock.Object, Mock.Of<IManifestServiceFactory>(), Mock.Of<ILoggerService>());
            command.Options.Manifest = Path.Combine(tempFolderContext.Path, "manifest.json");
            command.Options.MatrixType = MatrixType.PlatformDependencyGraph;
        ...
        private static GenerateBuildMatrixCommand CreateCommand() =>
            new(CreateManifestInfoProviderMock().Object, Mock.Of<IImageCacheService>(), Mock.Of<IManifestServiceFactory>(), Mock.Of<ILoggerService>());
    }
}
